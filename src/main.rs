...
fn main() {
    ...
    let input_filepath = opts.get_one::<String>("input_file")
        .expect("Error: Input file argument is missing. Please provide a valid file path.");
    let output_directory = opts.get_one::<String>("output_directory")
...
fn get_opts() -> clap::ArgMatches {
    return Command::new("ExcelentImages")
...
fn get_filename(path: &str) -> Option<String> {
    ...
    path.file_stem().map(|os_str| os_str.to_string_lossy().to_string())
}
